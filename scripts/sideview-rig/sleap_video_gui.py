...
DEFAULT_DATA_DIR = os.path.join(_ROOT_DIR, "data", "sideview_data", "proofreading")
DEFAULT_VIDEO = os.path.join(_ROOT_DIR, "data", "sideview_data", "trim.mp4")
...
class SleapVideoGUI:
    ...
    def _update_geom_model(self, ts: TrackState, dists):
        ...
        for i, d in enumerate(dists):
            if not np.isfinite(d):
                continue
            ts.geom_count[i] += 1
            delta = d - ts.geom_mean[i]
            ts.geom_mean[i] += delta / ts.geom_count[i]
            delta2 = d - ts.geom_mean[i]
            ts.geom_m2[i] += delta * delta2
    ...
    def _geom_error(self, ts: TrackState, dists):
        ...
        for i, d in enumerate(dists):
            ...
            if ts.geom_count[i] < 2:
                continue
            var = ts.geom_m2[i] / max(1, ts.geom_count[i] - 1)
    ...
    def _predict_position(self, ts: TrackState):
        ...
        if ts.prev_centroid is not None:
            prev_x, prev_y = ts.prev_centroid
            dt = 1
        elif ts.prev_manual_point is not None:
